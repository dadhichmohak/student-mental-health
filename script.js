/**
 * Student Mental Health — Frontend Logic
 *
 * Handles form validation, API calls, gauge animation, and result display.
 * Communicates with the FastAPI backend at /predict.
 *
 * Score bands (4 levels):
 *   0 – 3.9  Strained
 *   4 – 5.9  Moderate
 *   6 – 7.9  Balanced
 *   8 – 10   Strong
...
(() => {
  ...
  // Backend URL — auto-detects local vs deployed
  ...
  // Gauge arc length: semicircle with radius 100 → pi * 100 ≈ 314
  const GAUGE_ARC_LENGTH = 314;

  // ---------------------------------------------------------
  // Draw tick marks on the gauge (0, 2, 4, 6, 8, 10)
  // ---------------------------------------------------------
  function drawTicks() {
  ...
  // ---------------------------------------------------------
  // Segmented control for stress level (Low / Medium / High / Very High)
  // ---------------------------------------------------------
  const segGroup = document.getElementById("stress_level_group");
  const stressHiddenInput = document.getElementById("stress_level");
  const stressHints = document.querySelectorAll(".stress-hint");

  function updateStressHint(value) {
    stressHints.forEach((h) => {
      h.classList.toggle("active", h.dataset.for === value);
    });
  }
  ...
  segGroup.querySelectorAll(".seg-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      updateStressHint(btn.dataset.value);
    });
  });
  ...
  // Mirrors what pydantic checks on the backend
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // Gather form data into the shape the API expects
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // Map score to a signal band with context text
  // ---------------------------------------------------------
  function bandFor(score) {
    ...
    if (score < 6) {
      return {
        ...
        context: "Your rhythm shows some balance, but there is room to build healthier habits. Prioritize sleep and reduce screen time.",
      };
    }
    if (score < 8) {
      return {
  ...
  // ---------------------------------------------------------
  // Render the result on the gauge + score display
  // ---------------------------------------------------------
  function renderResult(score) {
    ...
    // Animate gauge fill — reset first, then set target offset
  ...
  // and map errors to form fields where possible
  // ---------------------------------------------------------
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderError(
        ...
        `Couldn't connect to the backend. Make sure it's running (python -m uvicorn main:app --port 2200 --reload) and try again.`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  // Reset buttons take you back to the idle state
